...
N=25; % number of points in the grid
...
vx=-(3*v0*a^3/2)*z.*x./sqrt(x.^2+z.^2).^5;
vz=v0+(v0*a^3/2)*(1 ./sqrt(x.^2+z.^2).^3-3*(z.^2)./sqrt(x.^2+z.^2).^5);
...
for i=1:100
  ...
  hold off
  %--------------Print plot
  filename=sprintf('E:\\Digipen\\PHY250\\2021\\Homeworks\\Final_project\\Fluid\\animation\\%05d.png',i);
  print(filename);
endfor
